...
export const App = () => {
  ...
  const handleSearchShoe = async () => {
    ...
    try {
      const response = await fetch("/api/search-shoe", {
        ...
        body: JSON.stringify({ shoe: shoe }), // Sending the video URL in the request body
      });
  ...
  return (
    <MainDiv>
      ...
      <SearchBarContainer>
        <Input value={shoe} onChange={shoeChange} />
